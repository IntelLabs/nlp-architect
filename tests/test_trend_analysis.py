...
import pytest
import os
from solutions.trend_analysis import topic_extraction, trend_analysis
...
def output_folder_path():
    return os.path.join(current_dir, os.path.pardir, 'solutions/trend_analysis/data')
...
def test_topic_extraction(input_data_path, output_folder_path):
    ...
    assert os.path.isfile('reference_corpus.csv')
    assert os.path.isfile('target_corpus.csv')
...
def test_trend_analysis(filter_data_path, graph_data_path):
    trend_analysis.analyze('target_corpus.csv', 'reference_corpus.csv',
                           'target_corpus.csv', 'reference_corpus.csv')
    assert os.path.isfile(filter_data_path)
    assert os.path.isfile(graph_data_path)


def teardown_module():
    os.remove('reference_corpus.csv')
    os.remove('target_corpus.csv')
