...
def test_tagging_procedure_sanity():
    ...
    idcnn_softmax_args = PARSER.parse_args(
        [
            ...
            '--lr', str(LEARNING_RATE), '-e', str(EPOCHS), '--train_filename',
            TRAIN_FILENAME, '--dev_filename', TRAIN_FILENAME, '--test_filename',
            TRAIN_FILENAME, '--model_type', model_type, '--overwrite_output_dir'
        ])
    ...
    idcnn_crf_args = PARSER.parse_args(
        [
            ...
            '--lr', str(LEARNING_RATE), '-e', str(EPOCHS), '--train_filename',
            TRAIN_FILENAME, '--dev_filename', TRAIN_FILENAME, '--test_filename',
            TRAIN_FILENAME, '--model_type', model_type, '--use_crf',
    ...
    lstm_crf_args = PARSER.parse_args(
        [
            ...
            '--lr', str(LEARNING_RATE), '-e', str(EPOCHS), '--train_filename',
            TRAIN_FILENAME, '--dev_filename', TRAIN_FILENAME, '--test_filename',
            TRAIN_FILENAME, '--model_type', model_type, '--use_crf', '--overwrite_output_dir'
        ])
