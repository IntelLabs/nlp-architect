...
from tests.cdc.test_utils import (
    get_embedd_mentions,
    get_wiki_mentions,
    get_compute_mentions,
...
def test_compute_relations():
    ...
    assert not compute.extract_all_relations(mentions[0], mentions[0]).isdisjoint(
        set([RelationType.EXACT_STRING])
    )

    assert not compute.extract_all_relations(mentions[0], mentions[1]).isdisjoint(
        set([RelationType.SAME_HEAD_LEMMA, RelationType.FUZZY_HEAD_FIT])
    )

    assert (
        compute.extract_all_relations(mentions[0], mentions[2]).pop()
...
def test_wordnet_relations():
    ...
    assert not wordnet.extract_all_relations(mentions[0], mentions[1]).isdisjoint(
        set([RelationType.WORDNET_DERIVATIONALLY])
    )

    assert (
        wordnet.extract_all_relations(mentions[0], mentions[2]).pop()
        == RelationType.NO_RELATION_FOUND
    )
