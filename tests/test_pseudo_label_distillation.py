...
def test_bert_seq_tag(data_dir, output_path):
    ...
    args = parser.parse_args(['--data_dir',data_dir, '--output_dir',output_path,
    '--model_name_or_path','bert-base-cased','--model_type','bert',
    '--train_file_name', 'labeled.txt', '--num_train_epoch', '1', '--per_gpu_train_batch_size', '50', '--overwrite_output_dir'])
...
def test_distillation_pseudo(data_dir, teacer_model_path, output_path):
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    args = parser.parse_args(['--data_dir',data_dir, '--output_dir',out_dir,
    '--teacher_model_path',teacer_model_path,'--teacher_model_type','bert',
    '--labeled_train_file','labeled.txt',
    '--unlabeled_train_file', 'unlabeled.txt', '-e', '10', '--overwrite_output_dir'])
    train_proc.run_procedure(args)
...
def test_split_dataset(data_dir, num_of_examples, labeled_precentage, unlabeled_precentage):
    if os.path.exists(data_dir):
        ...
        processor.split_dataset(dataset='train.txt',labeled=math.ceil(num_of_examples * labeled_precentage),
