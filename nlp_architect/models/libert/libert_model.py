...
from path_patterns import PatternPredictor, AspectMatcher
...
class LiBertConfig(BertConfig):
    ...
    def add_extra_args(self, hparams):
        ...
        # set hidden states of which layers to use in auxliary prediction 
        if not hparams.auxiliary_layers:
            self.auxiliary_layers = [self.num_hidden_layers]
        elif hparams.auxiliary_layers is "all":
            self.auxiliary_layers = list(range(1,self.num_hidden_layers+1))
        else:
            self.auxiliary_layers = hparams.auxiliary_layers

class LiBertForToken(InhouseBertForABSA):
    def __init__(self, config):
        super(LiBertForToken, self).__init__(config)
        # only deviation from InhouseBertForABSA is in underlyig BERT model (LiBertModel instead of BertModel)
        self.bert = LiBertModel(config)

    def call_bert(self, inputs, parse=None, **kwargs): # for adding kwargs with default values
        return self.bert(inputs, parse=parse, **kwargs)   
