...
import absa_utils
from kibertv2_model import CustomBertForTokenClassification, CustomBertConfig
...
class BertForToken(pl.LightningModule):
    ...
    def prepare_data(self):
        ...
        for mode in "train", "dev", "test":
            ...
            if os.path.exists(cached_features_file) and not self.hparams.overwrite_cache:
                ...
                self.dpr_encoder = DPRQuestionEncoder.from_pretrained('facebook/dpr-question_encoder-single-nq-base')
                ...
                new_features = []
                print("Computing DPR Embeddings...")
                for e, f in tqdm(zip(examples, features), total=len(examples)):
                    review_embedding = self.dpr_encoder(**self.dpr_tokenizer(' '.join(e.words), return_tensors="pt"))[0].detach()
                    new_features.append((f, review_embedding))
                new_features = zip(*new_features)
    ...
    def load_dataset(self, mode, batch_size):
        ...
        cached_features_file = self._feature_file(mode)
        log.debug("Loading features from cached file %s", cached_features_file)
        features, review_embeddings = torch.load(cached_features_file)
        ...
        if self.model_type is CustomBertForTokenClassification:
            #### Attach DPR embeddings ###
            if self.hparams.rnd_init:
                log.debug("Using random embedding for each example.")
                random_embeddings = [torch.rand(1, 768) for _ in passage_embeddings]  # TODO make k a hparam and modify this and previous function
                tensors.append(torch.stack(random_embeddings))
            else:
                log.debug("Using DPR embedding for each example.")
                tensors.append(torch.stack(review_embeddings))
    ...
    def map_to_inputs(batch):
        inputs = {"input_ids": batch[0], "attention_mask": batch[1], "token_type_ids": batch[2],
                  "labels": batch[3]}
        if len(batch) >= 5:
            inputs["review_embeddings"] = batch[4] 
        return inputs
    ...
    def training_step(self, batch, _):
        ...
        if self.model_type is CustomBertForTokenClassification:
            asp_loss = outputs[-3]
            reconstruction_loss = outputs[-2]
            return_dict.update({'asp_loss': asp_loss, 'reconstruction_loss': reconstruction_loss})
    ...
    def training_epoch_end(self, outputs):
        ...
        if self.model_type is CustomBertForTokenClassification:
            ...
            avg_reconstruction_loss = torch.stack([x['reconstruction_loss'] for x in outputs]).mean()
            return_dict.update({'asp_loss': avg_asp_loss, 'reconstruction_loss': avg_reconstruction_loss})
            if self.hparams.gamma:
