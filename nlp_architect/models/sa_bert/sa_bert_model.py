...
class SaBertExtEncoder(BertEncoder):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList([SaBertExtLayer(config, layer_num) for layer_num in range(config.num_hidden_layers)])
        self.li_layer = config.extra_args['li_layer']
        self.all_layers = config.extra_args['all_layers']
        self.layers_range = config.extra_args['layers_range']
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None, head_probs=None):
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            #layer_outputs = layer_module(hidden_states, attention_mask, head_mask[i], 
            #            head_probs if i == self.li_layer else None)
...
class SaBertExtSelfAttention(BertSelfAttention):
    def __init__(self, config, layer_num):
        ...
        self.replace_final = config.extra_args['replace_final']
        self.random_init = config.extra_args['random_init']
        self.duplicated_rels = config.extra_args['duplicated_rels']
        self.transpose = config.extra_args['transpose']

        #self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
        if  (layer_num == config.extra_args['li_layer'] or  config.extra_args['all_layers'] is True \
             or layer_num in config.extra_args['layers_range']): 
