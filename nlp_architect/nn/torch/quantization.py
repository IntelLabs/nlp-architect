...
class QuantizedLinear(nn.Linear):
    ...
    def __init__(self, *args, activation_bits=8, weight_bits=8, requantize_output=True, ema_decay=0.9999, start_step=0, mode='none', **kwargs):
        super().__init__(*args, **kwargs)
        if activation_bits < 2 or weight_bits < 2:
            raise ValueError(f"activation_bits={activation_bits} and weight_bits={weight_bits} must be higher than 1 ")
    ...
    def training_quantized_forward(self, input):
        ...
        out = F.linear(_fake_quantize(input, input_scale, self.activation_bits), self.fake_quantized_weight, self.bias)
    ...
    def extra_repr(self):
        return super().extra_repr() + f', mode={self.mode}, activation_bits={self.activation_bits}, weight_bits={self.weight_bits}, ema_decay={self.ema_decay if self.mode == QuantizationMode.EMA else None}'
